...
class SaleContract(models.Model):
    ...
    sale_order_auto_cancel = fields.Boolean(
        default=True,
        tracking=True,
        string='Cancel previous SOs when creating new one within this contract'
    )
    ...
    contract_line_ids = fields.One2many('sale.order.line', string='Contract Lines', copy=False, compute='_compute_contract_lines')
    ...
    @api.depends('contract_line_ids', 'contract_line_ids.product_uom_qty', 'contract_line_ids.price_subtotal')
                ...
                for subscription in subs:
                    ...
                    try:
                        if subscription.date_end and subscription.recurring_next_date >= subscription.date_end:
                            if not automatic:
                                raise UserError('Contract expired')
                            return
    ...
    def _compute_contract_lines(self):
        for contract in self:
            order_lines = contract.sale_order_ids.filtered(
                lambda order: order.state == 'sale'
            ).mapped('order_line')

            if order_lines:
                contract.contract_line_ids = [(4, line.id) for line in order_lines]
            else:
